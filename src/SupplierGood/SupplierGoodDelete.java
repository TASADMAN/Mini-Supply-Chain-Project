...
public class SupplierGoodDelete extends JPanel implements ActionListener {
	JTextField tfID;

	Main W11Main;

	public SupplierGoodDelete(Main ex11MainParam) {

		W11Main = ex11MainParam;

		setPreferredSize(new Dimension(W11Main.JMenu.FRAME_HEIGHT, W11Main.JMenu.FRAME_HEIGHT));
		setLayout(null);
		JLabel lbHeader = new JLabel("SupplierGoodDelete");
		...
		yl += 25;
		yt += 25;
		JButton btAdd = new JButton(main_Menu.DELETE_SUP);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals(main_Menu.DELETE_SUPGOOD)) {
			String strID = tfID.getText();
			try {
				int supg = Integer.parseInt(strID);
				SupplierGoodDAO SupDAO = new SupplierGoodDAO();
				SupDAO.deleteSupplierGood(supg);
